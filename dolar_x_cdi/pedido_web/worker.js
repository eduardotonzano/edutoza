...
async function tratarPedido(request, env) {
  ...
  const resultado = await dispararWorkflow(env, email, dataFim, pretoBranco);
  if (!resultado.ok) {
    return respostaErro(
...
async function dispararWorkflow(env, email, dataFim, pretoBranco) {
