...
async function tratarPedido(request, env) {
  ...
  const pretoBranco = corpo?.preto_branco === true;

  const spreadCdi = validarSpread(corpo?.spread_cdi, 0.04);
  if (spreadCdi === null) {
    return respostaErro("Spread do CDI inválido (use um valor entre 0 e 50%).", 400);
  }
  const spreadDolar = validarSpread(corpo?.spread_dolar, 0.035);
  if (spreadDolar === null) {
    return respostaErro("Spread do dólar inválido (use um valor entre 0 e 50%).", 400);
  }
  ...
  const resultado = await dispararWorkflow(env, email, dataFim, pretoBranco, spreadCdi, spreadDolar);
  if (!resultado.ok) {
    return respostaErro(
...
function validarSpread(valor, padrao) {
...
async function dispararWorkflow(env, email, dataFim, pretoBranco, spreadCdi, spreadDolar) {
      ...
      `https://api.github.com/repos/${OWNER}/${REPO}/actions/workflows/${WORKFLOW_FILE}/dispatches`,
      {
        ...
        body: JSON.stringify({
          ...
          inputs: {
            email_destino: email,
            data_fim: dataFim || "",
            preto_branco: pretoBranco ? "true" : "false",
            spread: String(spreadCdi),
            spread_dolar: String(spreadDolar),
          },
        }),
      }
    );
